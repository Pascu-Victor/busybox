...
static NOINLINE unsigned int parse_meminfo(struct globals* g) {
    ...
    while (fgets(buf, sizeof(buf), fp)) {
        if (sscanf(buf, "Cached: %lu %*s\n", &g->cached_kb) == 1)
            if (--seen_cached_and_available_and_reclaimable == 0) break;
...
int free_main(int argc UNUSED_PARAM, char** argv IF_NOT_DESKTOP(UNUSED_PARAM)) {
    ...
    printf(
        ...
        "Mem:   ",
        "total", "used", "free", "shared", "buff/cache", "available" /* swap and total don't have these columns */
    );
    ...
    printf("%12s%12s%12s", scale(&G, info.totalram),     // total
    ...
    if (!seen_available) {
        ...
        printf("%12s%12s%12s\n" + 4, scale(&G, info.totalram - cached_plus_free),  // used
