...
#include <selinux/selinux.h>
    ...
    ((UCLIBC_VERSION >= KERNEL_VERSION(0, 9, 32) && UCLIBC_VERSION < KERNEL_VERSION(0, 9, 34) && defined __UCLIBC_HAS_UTMPX__) || \
...
#define MAXINT(T) (T)(((T) - 1) > 0 ? (T) - 1 : (T) ~((T)1 << (sizeof(T) * 8 - 1)))

#define MININT(T) (T)(((T) - 1) > 0 ? (T)0 : ((T)1 << (sizeof(T) * 8 - 1)))
 ...
 */
#define xrealloc_vector(vector, shift, idx) xrealloc_vector_helper((vector), (sizeof((vector)[0]) << 8) + (shift), (idx))
...
int recursive_action(const char* fileName, unsigned flags,
                     int FAST_FUNC (*fileAction)(struct recursive_state* state, const char* fileName, struct stat* statbuf),
                     int FAST_FUNC (*dirAction)(struct recursive_state* state, const char* fileName, struct stat* statbuf),
                     void* userData) FAST_FUNC;

/* Simpler version: call a function on each dirent in a directory */
int iterate_on_dir(const char* dir_name, int FAST_FUNC (*func)(const char*, struct dirent*, void*), void* private) FAST_FUNC;
...
enum {
     ...
     */
    BB_FATAL_SIGS = (int)(0 + (1LL << SIGHUP) + (1LL << SIGINT) + (1LL << SIGTERM) + (1LL << SIGPIPE)  // Write to pipe with no readers
...
uoff_t FAST_FUNC get_volume_size_in_bytes(int fd, const char* override, unsigned override_units, int extend);
...
static ALWAYS_INLINE void free_connection(connection_t* conn) { free(conn); }
...
static ALWAYS_INLINE void free_ioloop_state(ioloop_state_t* io) { free(io); }
static ALWAYS_INLINE void ioloop_decrease_current_timeout(ioloop_state_t* io, unsigned n) {
    if (io->current_iteration_timeout > n) io->current_iteration_timeout = n;
}
...
struct BUG_too_small {
    ...
    char BUG_family_t_too_small[(0 | AF_UNSPEC | AF_INET | AF_INET6 | AF_UNIX
#ifdef AF_PACKET
                                 | AF_PACKET
#endif
#ifdef AF_NETLINK
...
ssize_t xsendto(int s, const void* buf, size_t len, const struct sockaddr* to, socklen_t tolen) FAST_FUNC;
...
void FAST_FUNC tls_handshake_as_server(tls_state_t* tls, const char* pem_filename);
...
ssize_t recv_from_to(int fd, void* buf, size_t len, int flags, struct sockaddr* from, struct sockaddr* to, socklen_t sa_size) FAST_FUNC;
...
#define SEAMLESS_COMPRESSION                                                                                                         \
    (0 || ENABLE_FEATURE_SEAMLESS_XZ || ENABLE_FEATURE_SEAMLESS_LZMA || ENABLE_FEATURE_SEAMLESS_BZ2 || ENABLE_FEATURE_SEAMLESS_GZ || \
...
const char* make_human_readable_str(unsigned long long size, unsigned long block_size, unsigned long display_unit) FAST_FUNC;
...
#define BB_EXECLP(prog, cmd, ...)                                                           \
    do {                                                                                    \
        if (find_applet_by_name(prog) >= 0) execlp(bb_busybox_exec_path, cmd, __VA_ARGS__); \
...
#define xvfork()                                                       \
    ({                                                                 \
        ...
        if (bb__xvfork_pid < 0) bb_simple_perror_msg_and_die("vfork"); \
        bb__xvfork_pid;                                                \
    })
#if BB_MMU
...
#else
#define getopt32long(argv, optstring, longopts, ...) getopt32(argv, optstring, ##__VA_ARGS__)
#endif
...
#define remove_pidfile(path)             \
    do {                                 \
        if (wrote_pidfile) unlink(path); \
    } while (0)
#else
 ...
 */
static inline void __attribute__((deprecated("use bb_simple_error_msg instead"))) bb_not_simple_error_msg(const char* s) {
    bb_simple_error_msg(s);
}
static inline void __attribute__((deprecated("use bb_simple_error_msg_and_die instead"))) NORETURN
bb_not_simple_error_msg_and_die(const char* s) {
    bb_simple_error_msg_and_die(s);
}
static inline void __attribute__((deprecated("use bb_simple_perror_msg instead"))) bb_not_simple_perror_msg(const char* s) {
    bb_simple_perror_msg(s);
}
static inline void __attribute__((deprecated("use bb_simple_perror_msg_and_die instead"))) NORETURN
bb_not_simple_perror_msg_and_die(const char* s) {
    bb_simple_perror_msg_and_die(s);
}
static inline void __attribute__((deprecated("use bb_simple_herror_msg instead"))) bb_not_simple_herror_msg(const char* s) {
    bb_simple_herror_msg(s);
}
static inline void __attribute__((deprecated("use bb_simple_herror_msg_and_die instead"))) NORETURN
bb_not_simple_herror_msg_and_die(const char* s) {
    bb_simple_herror_msg_and_die(s);
}
static inline void __attribute__((deprecated("use bb_simple_info_msg instead"))) bb_not_simple_info_msg(const char* s) {
    bb_simple_info_msg(s);
}
 ...
 */
int set_loop(char** devname, const char* file, unsigned long long offset, unsigned long long sizelimit, unsigned flags) FAST_FUNC;
...
extern context_t set_security_context_component(security_context_t cur_context, char* user, char* role, char* type, char* range) FAST_FUNC;
...
#define update_passwd(filename, username, data, member) update_passwd(filename, username, data)
#endif
extern int update_passwd(const char* filename, const char* username, const char* data, const char* member) FAST_FUNC;
...
#endif
#endif
#if (ENABLE_FEATURE_USERNAME_COMPLETION || ENABLE_FEATURE_EDITING_FANCY_PROMPT) && (ENABLE_SHELL_ASH || ENABLE_SHELL_HUSH)
...
#define read_line_input(state, prompt, command, maxsize) read_line_input(prompt, command, maxsize)
#endif
...
enum {
    ...
    PSSCAN_ARGVN = (1 << 16) * (ENABLE_KILLALL || ENABLE_PGREP || ENABLE_PKILL || ENABLE_PIDOF || ENABLE_SESTATUS),
...
#if HMAC_ONLY_SHA256
#define hmac_begin(ctx, key, key_size, begin) hmac_begin(ctx, key, key_size)
#endif
void FAST_FUNC hmac_begin(hmac_ctx_t* ctx, const uint8_t* key, unsigned key_size, md5sha_begin_func* begin);
static ALWAYS_INLINE void hmac_hash(hmac_ctx_t* ctx, const void* in, size_t len) { md5sha_hash(&ctx->hashed_key_xor_ipad, in, len); }
unsigned FAST_FUNC hmac_end(hmac_ctx_t* ctx, uint8_t* out);
#if HMAC_ONLY_SHA256
#define hmac_block(key, key_size, begin, in, sz, out) hmac_block(key, key_size, in, sz, out)
#endif
...
int print_flags_separated(const int* masks, const char* labels, int flags, const char* separator) FAST_FUNC;
...
#define bb_progress_free(p)                                      \
    do {                                                         \
        if (ENABLE_UNICODE_SUPPORT) free((char*)((p)->curfile)); \
        (p)->curfile = NULL;                                     \
    } while (0)
void bb_progress_init(bb_progress_t* p, const char* curfile) FAST_FUNC;
int bb_progress_update(bb_progress_t* p, uoff_t beg_range, uoff_t transferred, uoff_t totalsize) FAST_FUNC;
...
#else
#define ASSIGN_CONST_PTR(pptr, v)                                     \
    do {                                                              \
        *(void**)(pptr) = (void*)(v);                                 \
...
#define SET_OFFSET_PTR_TO_GLOBALS(x) ASSIGN_CONST_PTR(&ptr_to_globals, (char*)(x) + ARCH_GLOBAL_PTR_OFF)
#define OFFSET_PTR_TO_GLOBALS ((struct globals*)((char*)ptr_to_globals - ARCH_GLOBAL_PTR_OFF))
#else
...
#define FETCH_LE32(field) (sizeof(field) == 4 ? SWAP_LE32(field) : BUG_wrong_field_size())
...
#define isblank(a)                                 \
    ({                                             \
        unsigned char bb__isblank = (a);           \
        bb__isblank == ' ' || bb__isblank == '\t'; \
    })
#define iscntrl(a)                                \
    ({                                            \
        unsigned char bb__iscntrl = (a);          \
        bb__iscntrl < ' ' || bb__iscntrl == 0x7f; \
    })
...
#define isspace(a)                                           \
    ({                                                       \
        ...
        bb__isspace == (' ' - 9) || bb__isspace <= (13 - 9); \
    })
...
#define BBUNIT_DEFINE_TEST(NAME)                                                                         \
    ...
    static void INIT_FUNC bbunit_##NAME##_register(void) { bbunit_registertest(&bbunit_##NAME##_elem); } \
    static void bbunit_##NAME##_test(void)
...
#define BBUNIT_PRINTASSERTFAIL                                                            \
    do {                                                                                  \
        bb_error_msg("[ERROR] Assertion failed in file %s, line %d", __FILE__, __LINE__); \
    } while (0)
...
#define BBUNIT_ASSERT_FALSE(STATEMENT)                                             \
    do {                                                                           \
        if ((STATEMENT)) {                                                         \
            BBUNIT_PRINTASSERTFAIL;                                                \
            bb_error_msg("[ERROR] Statement '%s' evaluated to true!", #STATEMENT); \
            BBUNIT_ASSERTION_FAILED;                                               \
        }                                                                          \
    } while (0)
...
#define BBUNIT_ASSERT_TRUE(STATEMENT)                                               \
    do {                                                                            \
        if (!(STATEMENT)) {                                                         \
            BBUNIT_PRINTASSERTFAIL;                                                 \
            bb_error_msg("[ERROR] Statement '%s' evaluated to false!", #STATEMENT); \
            BBUNIT_ASSERTION_FAILED;                                                \
        }                                                                           \
    } while (0)
