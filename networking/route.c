...
static const uint16_t flags_ipvx[] ALIGN2 = {/* MUST match tbl_ipvx[] values above. */
                                             ...
                                             RTF_REJECT,
#endif
                                             RTF_MODIFIED, RTF_DYNAMIC, RTF_REINSTATE};
...
static NOINLINE void INET_setroute(int action, char** args) {
...

    {
        ...
        isnet = INET_resolve(target, (struct sockaddr_in*)&rt->rt_dst, (xflag & HOST_FLAG));
        if (isnet < 0) {
    ...
    while (*args) {
        ...
        if (k == KW_IPVx_GATEWAY) {
            ...
            isnet = INET_resolve(args_m1, (struct sockaddr_in*)&rt->rt_gateway, 1);
    ...
    if (mask_in_addr(*rt)) {
        ...
        if ((rt->rt_flags & RTF_HOST) && mask != 0xffffffff) {
            bb_error_msg_and_die("netmask %.8x and host route conflict", (unsigned int)mask);
        }
        if (mask & (mask + 1)) {
...
static const IF_NOT_FEATURE_IPV6(uint16_t flagvals[] ALIGN2 =) IF_FEATURE_IPV6(uint32_t flagvals[] ALIGN4 =){
    /* Must agree with flagchars[]. */
    RTF_UP,      RTF_GATEWAY,  RTF_HOST,  RTF_REINSTATE, RTF_DYNAMIC,   RTF_MODIFIED,
#if ENABLE_FEATURE_IPV6
    RTF_DEFAULT, RTF_ADDRCONF, RTF_CACHE, RTF_REJECT,    RTF_NONEXTHOP, /* this one doesn't fit into 16 bits */
#endif
};
...
static void INET6_displayroutes(void) {
    ...
    while (1) {
        int r;
        r = fscanf(fp, "%32s%x%*s%x%32s%x%x%x%x%s\n", addr6x + 14, &prefix_len, &slen, addr6x + 40 + 7, &metric, &refcnt, &use, &iflags,
                   iface);
        if (r != 9) {
            if ((r < 0) && feof(fp)) { /* EOF with no (nonspace) chars read. */
                break;
        ...
        while (1) {
            ...
            inet_pton(AF_INET6, addr6x + r, (struct sockaddr*)&snaddr6.sin6_addr);
            snaddr6.sin6_family = AF_INET6;
            naddr6 = INET6_rresolve((struct sockaddr_in6*)&snaddr6, 0x0fff /* Apparently, upstream never resolves. */
            );
            if (!r) { /* 1st pass */
...
///////:       "[-ne]"IF_FEATURE_IPV6(" [-A inet[6]]")" [{add|del|delete} [-net|-host] TARGET [netmask MASK] [gw GATEWAY] [metric N] [mss
///BYTES] [window BYTES] [irtt MSEC] [reject] [mod] [dyn] [reinstate] [[dev] IFACE]]"
///////too wordy
