 ...
 */
static int sp_256_cmp_equal_8(const sp_digit* a, const sp_digit* b) { return sp_256_cmp_8(a, b) == 0; }
...
#else
static void sp_256_sub_8_p256_mod(sp_digit* r) { sp_256_sub_8(r, r, p256_mod); }
#endif
...
static void sp_256_div2_8(sp_digit* r /*, const sp_digit* m*/) {
    ...
    int carry = 0;
    if (r[0] & 1) carry = sp_256_add_8(r, r, m);
...
static void sp_256_mont_dbl_8(sp_digit* r, const sp_digit* a /*, const sp_digit* m*/) {
    ...
    if (carry) sp_256_sub_8_p256_mod(r);
}
...
static void sp_512to256_mont_shift_8(sp_digit* r, sp_digit* a) { memcpy(r, a + 8, sizeof(*r) * 8); }
...
static void sp_512to256_mont_reduce_8(sp_digit* r, sp_digit* aa /*, const sp_digit* m, sp_digit mp*/) {
    ...
    if (carry != 0) sp_256_sub_8_p256_mod(r);
}
...
static void sp_512to256_mont_reduce_8(sp_digit* r, sp_digit* a /*, const sp_digit* m, sp_digit mp*/) {
    ...
    if (mp != 1) {
        ...
        if (word16th != 0) sp_256_sub_8_p256_mod(r);
...
static NOINLINE void sp_256_mont_mul_and_reduce_8(sp_digit* r, const sp_digit* a, const sp_digit* b
                                                  /*, const sp_digit* m, sp_digit mp*/) {
    sp_digit rr[2 * 8];
...
static void sp_256_mont_inv_8(sp_digit* r, sp_digit* a) {
    ...
    for (i = 254; i >= 0; i--) {
        ...
        if (i >= 224 || i == 192 || (i <= 95 && i != 1)) sp_256_mont_mul_8(r, r, a /*, p256_mod, p256_mp_mod*/);
    }
}
...
static void sp_256_map_8(sp_point* r, sp_point* p) {
    ...
    if (sp_256_cmp_8(r->x, p256_mod) >= 0) sp_256_sub_8_p256_mod(r->x);

    /* y /= z^3 */
    sp_256_mont_mul_and_reduce_8(r->y, p->y, t1 /*, p256_mod, p256_mp_mod*/);
    /* Reduce y to less than modulus */
    if (sp_256_cmp_8(r->y, p256_mod) >= 0) sp_256_sub_8_p256_mod(r->y);
...
static void sp_256_ecc_gen_k_8(sp_digit k[8]) {
    ...
    if (k[0] == 0) k[0] = 1;
    if (k[7] >= 0xffffffff) k[7] = 0xfffffffe;
}
...
void FAST_FUNC curve_P256_compute_premaster(const uint8_t* privkey32, const uint8_t* peerkey2x32, uint8_t* premaster32) {
    sp_digit privkey_sp[8];
...
void FAST_FUNC curve_P256_compute_pubkey_and_premaster(uint8_t* pubkey2x32, uint8_t* premaster32, const uint8_t* peerkey2x32) {
    sp_digit privkey[8];
