...
unsigned FAST_FUNC cap_name_to_number(const char* cap) {
    ...
    for (i = 0; i < ARRAY_SIZE(capabilities); i++) {
        if (strcasecmp(capabilities[i], cap) == 0) goto found;
    }
    bb_error_msg_and_die("unknown capability '%s'", cap);

found:
    if (!cap_valid(i)) bb_error_msg_and_die("unknown capability '%s'", cap);
    return i;
}
...
void FAST_FUNC getcaps(void* arg) {
    ...
    for (i = 0; i < ARRAY_SIZE(versions); i++) {
        ...
        if (capget(&caps->header, NULL) == 0) goto got_it;
    }
...
got_it:
    ...
    if (capget(&caps->header, caps->data) != 0) bb_simple_perror_msg_and_die("capget");
}
