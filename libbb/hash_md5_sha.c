...
static void cpuid_eax_ebx_ecx(unsigned* eax, unsigned* ebx, unsigned* ecx, unsigned* edx) {
    __asm__("cpuid" : "=a"(*eax), "=b"(*ebx), "=c"(*ecx), "=d"(*edx) : "0"(*eax), "1"(*ebx), "2"(*ecx));
}
...
#if defined(__i386__)
struct ASM_expects_76_shaNI {
    char t[1 - 2 * (offsetof(sha256_ctx_t, hash) != 76)];
};
#endif
#if defined(__x86_64__)
struct ASM_expects_80_shaNI {
    char t[1 - 2 * (offsetof(sha256_ctx_t, hash) != 80)];
};
#endif
#endif
#endif
...
static ALWAYS_INLINE uint32_t rotl32(uint32_t x, unsigned n) { return (x << n) | (x >> (32 - n)); }
// #define rotr32(x,n) (((x) >> (n)) | ((x) << (32 - (n))))
static ALWAYS_INLINE uint32_t rotr32(uint32_t x, unsigned n) { return (x >> n) | (x << (32 - n)); }
/* rotr64 in needed for sha512 only: */
// #define rotr64(x,n) (((x) >> (n)) | ((x) << (64 - (n))))
static ALWAYS_INLINE uint64_t rotr64(uint64_t x, unsigned n) { return (x >> n) | (x << (64 - n)); }

/* rotl64 only used for sha3 currently */
static ALWAYS_INLINE uint64_t rotl64(uint64_t x, unsigned n) { return (x << n) | (x >> (64 - n)); }
...
static void FAST_FUNC common64_end(md5_ctx_t* ctx, int swap_needed) {
    ...
    while (1) {
        ...
        if (remaining >= 8) {
            ...
            if (swap_needed) t = bb_bswap_64(t);
...
static void FAST_FUNC md5_process_block64(md5_ctx_t* ctx) {
    ...
    static const uint32_t C_array[] ALIGN4 = {
        ...
        /* round 2 */
        0xf61e2562, 0xc040b340, 0x265e5a51, 0xe9b6c7aa, 0xd62f105d, 0x02441453, 0xd8a1e681, 0xe7d3fbc8, 0x21e1cde6, 0xc33707d6, 0xf4d50d87,
        0x455a14ed, 0xa9e3e905, 0xfcefa3f8, 0x676f02d9, 0x8d2a4c8a,
        /* round 3 */
        0xfffa3942, 0x8771f681, 0x6d9d6122, 0xfde5380c, 0xa4beea44, 0x4bdecfa9, 0xf6bb4b60, 0xbebfbc70, 0x289b7ec6, 0xeaa127fa, 0xd4ef3085,
        0x4881d05, 0xd9d4d039, 0xe6db99e5, 0x1fa27cf8, 0xc4ac5665,
        /* round 4 */
        0xf4292244, 0x432aff97, 0xab9423a7, 0xfc93a039, 0x655b59c3, 0x8f0ccc92, 0xffeff47d, 0x85845dd1, 0x6fa87e4f, 0xfe2ce6e0, 0xa3014314,
    ...
    if (BB_BIG_ENDIAN)
        for (i = 0; i < 16; i++) words[i] = SWAP_LE32(words[i]);
...
#define RD1As(a, b, c, d, e, n, RCONST)     \
    "\n\
	##movl	4*" n                       \
    "(%esp), %esi	# n=0, W[0] already in %esi \n\
	movl	" c                         \
    ", %edi		# c                         \n\
	xorl	" d                         \
    ", %edi		# ^d                        \n\
...
#define RD1C(a, b, c, d, e, n)                                                                                                             \
    RD1Cs("%e" STR(a), "%e" STR(b), "%e" STR(c), "%e" STR(d), "%e" STR(e), STR(((n + 13) & 15)), STR(((n + 8) & 15)), STR(((n + 2) & 15)), \
          STR(((n) & 15)), STR(RCONST))
#undef RCONST
#define RCONST 0x5A827999
        RD1A(ax, bx, cx, dx, bp, 0) RD1B(bp, ax, bx, cx, dx, 1) RD1B(dx, bp, ax, bx, cx, 2) RD1B(cx, dx, bp, ax, bx, 3)
            RD1B(bx, cx, dx, bp, ax, 4) RD1B(ax, bx, cx, dx, bp, 5) RD1B(bp, ax, bx, cx, dx, 6) RD1B(dx, bp, ax, bx, cx, 7)
                RD1B(cx, dx, bp, ax, bx, 8) RD1B(bx, cx, dx, bp, ax, 9) RD1B(ax, bx, cx, dx, bp, 10) RD1B(bp, ax, bx, cx, dx, 11) RD1B(
                    dx, bp, ax, bx, cx, 12) RD1B(cx, dx, bp, ax, bx, 13) RD1B(bx, cx, dx, bp, ax, 14) RD1B(ax, bx, cx, dx, bp, 15)
                    RD1C(bp, ax, bx, cx, dx, 16) RD1C(dx, bp, ax, bx, cx, 17) RD1C(cx, dx, bp, ax, bx, 18) RD1C(bx, cx, dx, bp, ax, 19)
#define RD2s(a, b, c, d, e, n13, n8, n2, n, RCONST) \
    ...
    "			# b = rotl32(b,30)          \n\
"
#define RD2(a, b, c, d, e, n)                                                                                                  \
    RD2s("%e" STR(a), "%e" STR(b), "%e" STR(c), "%e" STR(d), "%e" STR(e), STR(((20 + n + 13) & 15)), STR(((20 + n + 8) & 15)), \
         STR(((20 + n + 2) & 15)), STR(((20 + n) & 15)), STR(RCONST))
#undef RCONST
#define RCONST 0x6ED9EBA1
                        RD2(ax, bx, cx, dx, bp, 0) RD2(bp, ax, bx, cx, dx, 1) RD2(dx, bp, ax, bx, cx, 2) RD2(cx, dx, bp, ax, bx, 3)
                            RD2(bx, cx, dx, bp, ax, 4) RD2(ax, bx, cx, dx, bp, 5) RD2(bp, ax, bx, cx, dx, 6) RD2(dx, bp, ax, bx, cx, 7) RD2(
                                cx, dx, bp, ax, bx, 8) RD2(bx, cx, dx, bp, ax, 9) RD2(ax, bx, cx, dx, bp, 10) RD2(bp, ax, bx, cx, dx, 11)
                                RD2(dx, bp, ax, bx, cx, 12) RD2(cx, dx, bp, ax, bx, 13) RD2(bx, cx, dx, bp, ax, 14)
                                    RD2(ax, bx, cx, dx, bp, 15) RD2(bp, ax, bx, cx, dx, 16) RD2(dx, bp, ax, bx, cx, 17) RD2(
                                        cx, dx, bp, ax, bx, 18) RD2(bx, cx, dx, bp, ax, 19)
...
#define RD3s(a, b, c, d, e, n13, n8, n2, n, RCONST) \
    ...
    "			# b = rotl32(b,30)          \n\
"
#define RD3(a, b, c, d, e, n)                                                                                                  \
    RD3s("%e" STR(a), "%e" STR(b), "%e" STR(c), "%e" STR(d), "%e" STR(e), STR(((40 + n + 13) & 15)), STR(((40 + n + 8) & 15)), \
         STR(((40 + n + 2) & 15)), STR(((40 + n) & 15)), STR(RCONST))
#undef RCONST
#define RCONST 0x8F1BBCDC
                                        RD3(ax, bx, cx, dx, bp, 0) RD3(bp, ax, bx, cx, dx, 1) RD3(dx, bp, ax, bx, cx, 2)
                                            RD3(cx, dx, bp, ax, bx, 3) RD3(bx, cx, dx, bp, ax, 4) RD3(ax, bx, cx, dx, bp, 5)
                                                RD3(bp, ax, bx, cx, dx, 6) RD3(dx, bp, ax, bx, cx, 7) RD3(cx, dx, bp, ax, bx, 8)
                                                    RD3(bx, cx, dx, bp, ax, 9) RD3(ax, bx, cx, dx, bp, 10) RD3(bp, ax, bx, cx, dx, 11)
                                                        RD3(dx, bp, ax, bx, cx, 12) RD3(cx, dx, bp, ax, bx, 13) RD3(bx, cx, dx, bp, ax, 14)
                                                            RD3(ax, bx, cx, dx, bp, 15) RD3(bp, ax, bx, cx, dx, 16)
                                                                RD3(dx, bp, ax, bx, cx, 17) RD3(cx, dx, bp, ax, bx, 18)
                                                                    RD3(bx, cx, dx, bp, ax, 19)
...
#define RD4Bs(a, b, c, d, e, n13, n8, n2, n, RCONST) \
    ...
    "			# b = rotl32(b,30)          \n\
"
#define RD4A(a, b, c, d, e, n)                                                                                                  \
    RD4As("%e" STR(a), "%e" STR(b), "%e" STR(c), "%e" STR(d), "%e" STR(e), STR(((60 + n + 13) & 15)), STR(((60 + n + 8) & 15)), \
          STR(((60 + n + 2) & 15)), STR(((60 + n) & 15)), STR(RCONST))
#define RD4B(a, b, c, d, e, n)                                                                                                  \
    RD4Bs("%e" STR(a), "%e" STR(b), "%e" STR(c), "%e" STR(d), "%e" STR(e), STR(((60 + n + 13) & 15)), STR(((60 + n + 8) & 15)), \
          STR(((60 + n + 2) & 15)), STR(((60 + n) & 15)), STR(RCONST))
#undef RCONST
#define RCONST 0xCA62C1D6
                                                                        RD4A(ax, bx, cx, dx, bp, 0) RD4A(bp, ax, bx, cx, dx, 1) RD4A(
                                                                            dx, bp, ax, bx, cx, 2) RD4A(cx, dx, bp, ax, bx, 3)
                                                                            RD4A(bx, cx, dx, bp, ax, 4) RD4A(ax, bx, cx, dx, bp, 5) RD4A(
                                                                                bp, ax, bx, cx, dx, 6) RD4A(dx, bp, ax, bx, cx, 7)
                                                                                RD4A(cx, dx, bp, ax, bx, 8) RD4A(bx, cx, dx, bp, ax, 9)
                                                                                    RD4A(ax, bx, cx, dx, bp, 10) RD4A(
                                                                                        bp, ax, bx, cx, dx, 11) RD4A(dx, bp, ax, bx, cx, 12)
                                                                                        RD4A(cx, dx, bp, ax, bx, 13)
                                                                                            RD4A(bx, cx, dx, bp, ax, 14)
                                                                                                RD4A(ax, bx, cx, dx, bp, 15)
                                                                                                    RD4A(bp, ax, bx, cx, dx, 16)
                                                                                                        RD4B(dx, bp, ax, bx, cx, 17)
                                                                                                            RD4B(cx, dx, bp, ax, bx, 18)
                                                                                                                RD4B(bx, cx, dx, bp, ax, 19)
...
struct ASM_expects_80 {
    char t[1 - 2 * (offsetof(sha1_ctx_t, hash) != 80)];
};
...
static void FAST_FUNC sha1_process_block64(sha1_ctx_t* ctx) {
    static const uint32_t rconsts[] ALIGN4 = {0x5A827999, 0x6ED9EBA1, 0x8F1BBCDC, 0xCA62C1D6};
    uint32_t W[16];
    uint32_t a, b, c, d, e;
 ...
 */
#if defined(__GNUC__) && defined(__i386__)
#define DO_NOT_TRY_PROPAGATING(m) __asm__("" : "+m"(m))
#else
...
#define OP(A, B, C, D, E, n)                                                                                          \
    do {                                                                                                              \
        ...
        if (n <= 15) work += W[n & 15] = SWAP_BE32(((uint32_t*)ctx->wbuffer)[n]);                                     \
        if (n >= 16) work += W[n & 15] = rotl32(W[(n + 13) & 15] ^ W[(n + 8) & 15] ^ W[(n + 2) & 15] ^ W[n & 15], 1); \
...
static void FAST_FUNC sha1_process_block64(sha1_ctx_t* ctx) {
     ...
     */
    for (i = 0; i < 16; i++) W[i] = W[i + 16] = SWAP_BE32(((uint32_t*)ctx->wbuffer)[i]);
...
static const sha_K_int sha_K[] ALIGN8 = {
    K(0x428a2f98d728ae22ULL), K(0x7137449123ef65cdULL), K(0xb5c0fbcfec4d3b2fULL), K(0xe9b5dba58189dbbcULL), K(0x3956c25bf348b538ULL),
    K(0x59f111f1b605d019ULL), K(0x923f82a4af194f9bULL), K(0xab1c5ed5da6d8118ULL), K(0xd807aa98a3030242ULL), K(0x12835b0145706fbeULL),
    K(0x243185be4ee4b28cULL), K(0x550c7dc3d5ffb4e2ULL), K(0x72be5d74f27b896fULL), K(0x80deb1fe3b1696b1ULL), K(0x9bdc06a725c71235ULL),
    K(0xc19bf174cf692694ULL), K(0xe49b69c19ef14ad2ULL), K(0xefbe4786384f25e3ULL), K(0x0fc19dc68b8cd5b5ULL), K(0x240ca1cc77ac9c65ULL),
    K(0x2de92c6f592b0275ULL), K(0x4a7484aa6ea6e483ULL), K(0x5cb0a9dcbd41fbd4ULL), K(0x76f988da831153b5ULL), K(0x983e5152ee66dfabULL),
    K(0xa831c66d2db43210ULL), K(0xb00327c898fb213fULL), K(0xbf597fc7beef0ee4ULL), K(0xc6e00bf33da88fc2ULL), K(0xd5a79147930aa725ULL),
    K(0x06ca6351e003826fULL), K(0x142929670a0e6e70ULL), K(0x27b70a8546d22ffcULL), K(0x2e1b21385c26c926ULL), K(0x4d2c6dfc5ac42aedULL),
    K(0x53380d139d95b3dfULL), K(0x650a73548baf63deULL), K(0x766a0abb3c77b2a8ULL), K(0x81c2c92e47edaee6ULL), K(0x92722c851482353bULL),
    K(0xa2bfe8a14cf10364ULL), K(0xa81a664bbc423001ULL), K(0xc24b8b70d0f89791ULL), K(0xc76c51a30654be30ULL), K(0xd192e819d6ef5218ULL),
    K(0xd69906245565a910ULL), K(0xf40e35855771202aULL), K(0x106aa07032bbd1b8ULL), K(0x19a4c116b8d2d0c8ULL), K(0x1e376c085141ab53ULL),
    K(0x2748774cdf8eeb99ULL), K(0x34b0bcb5e19b48a8ULL), K(0x391c0cb3c5c95a63ULL), K(0x4ed8aa4ae3418acbULL), K(0x5b9cca4f7763e373ULL),
    K(0x682e6ff3d6b2b8a3ULL), K(0x748f82ee5defb2fcULL), K(0x78a5636f43172f60ULL), K(0x84c87814a1f0ab72ULL), K(0x8cc702081a6439ecULL),
    K(0x90befffa23631e28ULL), K(0xa4506cebde82bde9ULL), K(0xbef9a3f7b2c67915ULL), K(0xc67178f2e372532bULL),
#if NEED_SHA512 /* [64]+ are used for sha384 and sha512 only */
    K(0xca273eceea26619cULL), K(0xd186b8c721c0c207ULL), K(0xeada7dd6cde0eb1eULL), K(0xf57d4f7fee6ed178ULL), K(0x06f067aa72176fbaULL),
    K(0x0a637dc5a2c898a6ULL), K(0x113f9804bef90daeULL), K(0x1b710b35131c471bULL), K(0x28db77f523047d84ULL), K(0x32caab7b40c72493ULL),
    K(0x3c9ebe0a15c9bebcULL), K(0x431d67c49c100d4cULL), K(0x4cc5d4becb3e42b6ULL), K(0x597f299cfc657e2aULL), K(0x5fcb6fab3ad6faecULL),
    K(0x6c44198c4a475817ULL),
#endif
};
#undef K
...
#define R1(x) (rotr32(x, 17) ^ rotr32(x, 19) ^ (x >> 10))
    ...
    for (t = 0; t < 16; ++t) W[t] = SWAP_BE32(words[t]);
    for (/*t = 16*/; t < 64; ++t) W[t] = R1(W[t - 2]) + W[t - 7] + R0(W[t - 15]) + W[t - 16];
...
#define R1(x) (rotr64(x, 19) ^ rotr64(x, 61) ^ (x >> 6))
    ...
    for (t = 0; t < 16; ++t) W[t] = SWAP_BE64(words[t]);
    for (/*t = 16*/; t < 80; ++t) W[t] = R1(W[t - 2]) + W[t - 7] + R0(W[t - 15]) + W[t - 16];
...
void FAST_FUNC sha1_begin(sha1_ctx_t* ctx) {
...
#if defined(__GNUC__) && (defined(__i386__) || defined(__x86_64__))
    {
        int ni = shaNI;
        if (!ni) ni = get_shaNI();
        if (ni > 0) ctx->process_block = sha1_process_block64_shaNI;
    }
#endif
#endif
}
...
static const uint32_t init256[] ALIGN4 = {
    0, 0, 0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a, 0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19,
};
#if NEED_SHA512
static const uint32_t init512_lo[] ALIGN4 = {
    0, 0, 0xf3bcc908, 0x84caa73b, 0xfe94f82b, 0x5f1d36f1, 0xade682d1, 0x2b3e6c1f, 0xfb41bd6b, 0x137e2179,
};
#endif /* NEED_SHA512 */
#if ENABLE_SHA384SUM
...
void FAST_FUNC sha256_begin(sha256_ctx_t* ctx) {
...
#if defined(__GNUC__) && (defined(__i386__) || defined(__x86_64__))
    {
        int ni = shaNI;
        if (!ni) ni = get_shaNI();
        if (ni > 0) ctx->process_block = sha256_process_block64_shaNI;
    }
#endif
#endif
}
...
void FAST_FUNC sha512_begin(sha512_ctx_t* ctx) {
    ...
    for (i = 0; i < 8 + 2; i++) tp[i] = ((uint64_t)(init256[i]) << 32) + init512_lo[i];
    /*ctx->total64[0] = ctx->total64[1] = 0; - already done */
}
...
void FAST_FUNC sha512_hash(sha512_ctx_t* ctx, const void* buffer, size_t len) {
    ...
    ctx->total64[0] += len;
    if (ctx->total64[0] < len) ctx->total64[1]++;
...
unsigned FAST_FUNC sha1_end(sha1_ctx_t* ctx, void* resbuf) {
    ...
    if (BB_LITTLE_ENDIAN) {
        unsigned i;
        for (i = 0; i < hash_size; ++i) ctx->hash[i] = SWAP_BE32(ctx->hash[i]);
    }
...
static unsigned FAST_FUNC sha512384_end(sha512_ctx_t* ctx, void* resbuf, unsigned outsize) {
    ...
    if (BB_LITTLE_ENDIAN) {
        unsigned i;
        for (i = 0; i < ARRAY_SIZE(ctx->hash); ++i) ctx->hash[i] = SWAP_BE64(ctx->hash[i]);
    }
    memcpy(resbuf, ctx->hash, outsize);
    return outsize;
}
unsigned FAST_FUNC sha512_end(sha384_ctx_t* ctx, void* resbuf) { return sha512384_end(ctx, resbuf, SHA512_OUTSIZE); }
#endif /* NEED_SHA512 */

#if ENABLE_SHA384SUM
unsigned FAST_FUNC sha384_end(sha384_ctx_t* ctx, void* resbuf) { return sha512384_end(ctx, resbuf, SHA384_OUTSIZE); }
#endif
...
static void split_halves(uint64_t* state) {
    ...
    for (i = 24; i >= 0; --i) {
        x0 = s32[0];
        t = (x0 ^ (x0 >> 1)) & 0x22222222;
        x0 = x0 ^ t ^ (t << 1);
        t = (x0 ^ (x0 >> 2)) & 0x0C0C0C0C;
        x0 = x0 ^ t ^ (t << 2);
        t = (x0 ^ (x0 >> 4)) & 0x00F000F0;
        x0 = x0 ^ t ^ (t << 4);
        t = (x0 ^ (x0 >> 8)) & 0x0000FF00;
        x0 = x0 ^ t ^ (t << 8);
        x1 = s32[1];
        t = (x1 ^ (x1 >> 1)) & 0x22222222;
        x1 = x1 ^ t ^ (t << 1);
        t = (x1 ^ (x1 >> 2)) & 0x0C0C0C0C;
        x1 = x1 ^ t ^ (t << 2);
        t = (x1 ^ (x1 >> 4)) & 0x00F000F0;
        x1 = x1 ^ t ^ (t << 4);
        t = (x1 ^ (x1 >> 8)) & 0x0000FF00;
        x1 = x1 ^ t ^ (t << 8);
...
static void sha3_process_block72(uint64_t* state) {
    ...
    */
    ...
    static const uint32_t IOTA_CONST_1[NROUNDS] ALIGN4 = {
        0x00000000UL, 0x00000089UL, 0x8000008bUL, 0x80008080UL, 0x0000008bUL, 0x00008000UL, 0x80008088UL, 0x80000082UL,
        0x0000000bUL, 0x0000000aUL, 0x00008082UL, 0x00008003UL, 0x0000808bUL, 0x8000000bUL, 0x8000008aUL, 0x80000081UL,
        0x80000081UL, 0x80000008UL, 0x00000083UL, 0x80008003UL, 0x80008088UL, 0x80000088UL, 0x00008000UL, 0x80008082UL,
    };
...
#define RhoPi(PI_LANE, ROT_CONST)                                                     \
    ...
    t1b = t0b;
            ...
            RhoPi(10, 1) RhoPi(7, 3) RhoPi(11, 6) RhoPi(17, 10) RhoPi(18, 15) RhoPi(3, 21) RhoPi(5, 28) RhoPi(16, 36) RhoPi(8, 45)
                RhoPi(21, 55) RhoPi(24, 2) RhoPi(4, 14) RhoPi(15, 27) RhoPi(23, 41) RhoPi(19, 56) RhoPi(13, 8) RhoPi(12, 25) RhoPi(2, 43)
...
#undef RhoPi
    ...
    static const uint16_t IOTA_CONST[NROUNDS] ALIGN2 = {
        ...
        0x808b, 0x008b, 0x8089, 0x8003, 0x8002, 0x0080, 0x800a, 0x000a, 0x8081, 0x8080, 0x0001, 0x8008,
    };
...
#undef RhoPi_twice
        ...
        /* Iota */
        state[0] ^= IOTA_CONST[round] | (uint32_t)((IOTA_CONST_bit31 << round) & 0x80000000) |
