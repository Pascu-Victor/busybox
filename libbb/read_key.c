...
int64_t FAST_FUNC read_key(int fd, char* buffer, int timeout) {
    ...
    static const char esccmds[] ALIGN1 = {
        '\x7f' | 0x80, KEYCODE_ALT_BACKSPACE, '\b' | 0x80, KEYCODE_ALT_BACKSPACE, 'd' | 0x80, KEYCODE_ALT_D,
         ...
         */
        'f' | 0x80, KEYCODE_ALT_RIGHT, 'b' | 0x80, KEYCODE_ALT_LEFT, 'O', 'A' | 0x80, KEYCODE_UP, 'O', 'B' | 0x80, KEYCODE_DOWN, 'O',
        'C' | 0x80, KEYCODE_RIGHT, 'O', 'D' | 0x80, KEYCODE_LEFT, 'O', 'H' | 0x80, KEYCODE_HOME, 'O', 'F' | 0x80, KEYCODE_END,
#if 0
	...
	//	ESC [ O 1 ; 6 P - Ctrl-Shift-F1
...
#endif
        '[', 'A' | 0x80, KEYCODE_UP, '[', 'B' | 0x80, KEYCODE_DOWN, '[', 'C' | 0x80, KEYCODE_RIGHT, '[', 'D' | 0x80, KEYCODE_LEFT,
        ...
        '[', '6', '~' | 0x80, KEYCODE_PAGEDOWN, '[', '7', '~' | 0x80, KEYCODE_HOME, /* vt100? linux vt? or what? */
	...
	//	ESC [ 2 4 ; 6 ~ - Ctrl-Shift-F12
        ...
        '[', '1', ';', '3', 'C' | 0x80, KEYCODE_ALT_RIGHT, '[', '1', ';', '3', 'D' | 0x80, KEYCODE_ALT_LEFT,
        //	'[','1',';','5','A' |0x80,KEYCODE_CTRL_UP   , - unused
        //	'[','1',';','5','B' |0x80,KEYCODE_CTRL_DOWN , - unused
        '[', '1', ';', '5', 'C' | 0x80, KEYCODE_CTRL_RIGHT, '[', '1', ';', '5', 'D' | 0x80, KEYCODE_CTRL_LEFT,
...
start_over:
...

    {
        ...
        n--;
        if (n) memmove(buffer, buffer + 1, n);
    ...
    while (n < KEYCODE_BUFFER_SIZE - 1) { /* 1 for count byte at buffer[-1] */
        ...
        if ((ENABLE_FEATURE_EDITING_ASK_TERMINAL || ENABLE_FEATURE_VI_ASK_TERMINAL || ENABLE_FEATURE_LESS_ASK_TERMINAL) && n >= 5 &&
            buffer[0] == '[' && buffer[n - 1] == 'R' && isdigit(buffer[1])) {
            char* end;
            unsigned long row, col;

            row = strtoul(buffer + 1, &end, 10);
            if (*end != ';' || !isdigit(end[1])) continue;
            col = strtoul(end + 1, &end, 10);
            if (*end != 'R') continue;
            if (row < 1 || col < 1 || (row | col) > 0x7fff) continue;
...
void FAST_FUNC read_key_ungets(char* buffer, const char* str, unsigned len) {
    ...
    if (len > KEYCODE_BUFFER_SIZE - 1 - cur_len) len = KEYCODE_BUFFER_SIZE - 1 - cur_len;
