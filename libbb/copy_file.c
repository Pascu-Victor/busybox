...
static int ask_and_unlink(const char* dest, int flags) {
    ...
    if (flags & FILEUTILS_INTERACTIVE) {
        ...
        if (!bb_ask_y_confirmation()) return 0; /* not allowed to overwrite */
    }
    if (unlink(dest) < 0) {
    ...
    if (flags & FILEUTILS_RMDEST)
        if (flags & FILEUTILS_VERBOSE) printf("removed '%s'\n", dest);
#endif
...
#define FLAGS_DEREF (flags & (FILEUTILS_DEREFERENCE + FILEUTILS_DEREFERENCE_L0))
    ...
    if ((FLAGS_DEREF ? stat : lstat)(source, &source_stat) < 0) {
        ...
        if (flags & (FILEUTILS_MAKE_SOFTLINK | FILEUTILS_MAKE_HARDLINK)) goto make_links;
    ...
    if (S_ISDIR(source_stat.st_mode)) {
        ...
        if (dest_exists) {
            ...
            if (!(flags & FILEUTILS_PRESERVE_STATUS)) mode = source_stat.st_mode & ~saved_umask;
        ...
        while ((d = readdir(dp)) != NULL) {
            ...
            if (new_source == NULL) continue;
            new_dest = concat_path_file(dest, d->d_name);
            if (copy_file(new_source, new_dest, flags & ~FILEUTILS_DEREFERENCE_L0) < 0) retval = -1;
            free(new_source);
            free(new_dest);
        }
        closedir(dp);

        if (!dest_exists && chmod(dest, source_stat.st_mode & ~saved_umask) < 0) {
    ...
    if (/* "cp thing1 thing2" without -R: just open and read() from thing1 */
        ...
        if (!S_ISREG(source_stat.st_mode)) new_mode = 0666;
...
#if ENABLE_SELINUX
        if ((flags & (FILEUTILS_PRESERVE_SECURITY_CONTEXT | FILEUTILS_SET_SECURITY_CONTEXT)) && is_selinux_enabled() > 0) {
            security_context_t con;
...
#endif
#if ENABLE_FEATURE_CP_REFLINK && defined(__linux__)
...
#define BTRFS_IOC_CLONE _IOW(BTRFS_IOCTL_MAGIC, 9, int)
...
#endif
        if (bb_copyfd_eof(src_fd, dst_fd) == -1) retval = -1;
        ...
        if (!S_ISREG(source_stat.st_mode)) return retval;
        goto preserve_mode_ugid_time;
    }
dont_cat:
    ...
    if (S_ISLNK(source_stat.st_mode)) {
        ...
        if (lpath) {
            ...
            if (flags & FILEUTILS_PRESERVE_STATUS)
                if (lchown(dest, source_stat.st_uid, source_stat.st_gid) < 0) bb_perror_msg("can't preserve %s of '%s'", "ownership", dest);
        }
        /* _Not_ jumping to preserve_mode_ugid_time:
         * symlinks don't have those */
        goto verb_and_exit;
    }
    if (S_ISBLK(source_stat.st_mode) || S_ISCHR(source_stat.st_mode) || S_ISSOCK(source_stat.st_mode) || S_ISFIFO(source_stat.st_mode)) {
...
preserve_mode_ugid_time:
    ...
    if (flags & FILEUTILS_PRESERVE_STATUS
        ...
        if (utimes(dest, times) < 0) bb_perror_msg("can't preserve %s of '%s'", "times", dest);
        if (chown(dest, source_stat.st_uid, source_stat.st_gid) < 0) {
            source_stat.st_mode &= ~(S_ISUID | S_ISGID);
            bb_perror_msg("can't preserve %s of '%s'", "ownership", dest);
        }
        if (chmod(dest, source_stat.st_mode) < 0) bb_perror_msg("can't preserve %s of '%s'", "permissions", dest);
    }
