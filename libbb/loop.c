...
char* FAST_FUNC query_loop(const char* device) {
    ...
    if (fd >= 0) {
        if (ioctl(fd, BB_LOOP_GET_STATUS, &loopinfo) == 0) {
            dev = xasprintf("%" OFF_FMT "u %s", (off_t)loopinfo.lo_offset, (char*)loopinfo.lo_file_name);
        }
        close(fd);
    }
...
int FAST_FUNC get_free_loop(void) {
    ...
    if (fd == -1) return fd - 1; /* -2: "no /dev/loop-control" */
...
static int set_loopdev_params(int lfd, int ffd, const char* file, unsigned long long offset, unsigned long long sizelimit, unsigned flags) {
    int rc;
#if ENABLE_TRY_LOOP_CONFIGURE || ENABLE_LOOP_CONFIGURE
...
#define loopinfo lconfig.info
    ...
    if (rc && errno == ENXIO) {
        ...
        if (rc == 0) return rc; /* SUCCESS! */
#if ENABLE_TRY_LOOP_CONFIGURE
        if (errno != EINVAL && errno != ENOTTY) return rc; /* error other than old kernel */
...
open_ffd:
    ...
    for (i = 0; i <= 0xfffff; i++) {
        if (!*device) {
            ...
            if (rc == -1) break; /* no free loop devices (or other error in LOOP_CTL_GET_FREE) */
            if (rc >= 0)         /* /dev/loop-control gave us the next free /dev/loopN */
                goto open_lfd;
        ...
        if (stat(try, &statbuf) != 0 || !S_ISBLK(statbuf.st_mode)) {
            if (ENABLE_FEATURE_MOUNT_LOOP_CREATE && errno == ENOENT && (!*device)) {
                /* Node doesn't exist, try to create it */
                if (mknod(dev, S_IFBLK | 0644, makedev(7, i)) == 0) goto open_lfd;
            }
