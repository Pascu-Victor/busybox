...
#include <sys/types.h>
#include <sys/un.h>
#if defined(__linux__) && (ENABLE_IFPLUGD || ENABLE_FEATURE_MDEV_DAEMON || ENABLE_UEVENT)
#include <linux/netlink.h>
#endif
#include "libbb.h"

int FAST_FUNC setsockopt_int(int fd, int level, int optname, int optval) { return setsockopt(fd, level, optname, &optval, sizeof(int)); }
...
int FAST_FUNC setsockopt_SOL_SOCKET_1(int fd, int optname) { return setsockopt_SOL_SOCKET_int(fd, optname, 1); }

void FAST_FUNC setsockopt_reuseaddr(int fd) { setsockopt_SOL_SOCKET_1(fd, SO_REUSEADDR); }
int FAST_FUNC setsockopt_broadcast(int fd) { return setsockopt_SOL_SOCKET_1(fd, SO_BROADCAST); }
int FAST_FUNC setsockopt_keepalive(int fd) { return setsockopt_SOL_SOCKET_1(fd, SO_KEEPALIVE); }
...
int FAST_FUNC setsockopt_bindtodevice(int fd, const char* iface) {
    ...
    if (r) bb_perror_msg("can't bind to interface %s", iface);
    return r;
}
#else
int FAST_FUNC setsockopt_bindtodevice(int fd UNUSED_PARAM, const char* iface UNUSED_PARAM) {
...
static len_and_sockaddr* get_lsa(int fd, int (*get_name)(int fd, struct sockaddr* addr, socklen_t* addrlen)) {
    ...
    if (get_name(fd, &lsa.u.sa, &lsa.len) != 0) return NULL;
...
len_and_sockaddr* FAST_FUNC get_sock_lsa(int fd) { return get_lsa(fd, getsockname); }

len_and_sockaddr* FAST_FUNC get_peer_lsa(int fd) { return get_lsa(fd, getpeername); }

void FAST_FUNC xconnect(int s, const struct sockaddr* s_addr, socklen_t addrlen) {
    if (connect(s, s_addr, addrlen) < 0) {
        if (ENABLE_FEATURE_CLEAN_UP) close(s);
        if (s_addr->sa_family == AF_INET)
            bb_perror_msg_and_die("%s (%s)", "can't connect to remote host", inet_ntoa(((struct sockaddr_in*)s_addr)->sin_addr));
...
static len_and_sockaddr* str2sockaddr(const char* host, int port, IF_FEATURE_IPV6(sa_family_t af, ) int ai_flags) {
    ...
    if (ENABLE_FEATURE_IPV6 && host[0] == '[') {
        ...
        if (!cp || (cp[1] != ':' && cp[1] != '\0')) {
            ...
            if (ai_flags & DIE_ON_ERROR) xfunc_die();
            return NULL;
        }
    } else {
        cp = strrchr(host, ':');
    ...
    while (1) {
        if (used_res->ai_family == AF_INET) break;
...
#if ENABLE_FEATURE_IPV6
len_and_sockaddr* FAST_FUNC host_and_af2sockaddr(const char* host, int port, sa_family_t af) { return str2sockaddr(host, port, af, 0); }
...
len_and_sockaddr* FAST_FUNC host2sockaddr(const char* host, int port) { return str2sockaddr(host, port, AF_UNSPEC, 0); }

len_and_sockaddr* FAST_FUNC xhost2sockaddr(const char* host, int port) { return str2sockaddr(host, port, AF_UNSPEC, DIE_ON_ERROR); }
...
int FAST_FUNC xsocket_type(len_and_sockaddr** lsap, int family, int sock_type) {
    ...
    if (family == AF_UNIX) len = sizeof(struct sockaddr_un);
#if ENABLE_FEATURE_IPV6
    if (family == AF_INET6) {
    done:
...
int FAST_FUNC xsocket_stream(len_and_sockaddr** lsap) { return xsocket_type(lsap, AF_UNSPEC, SOCK_STREAM); }
...
int FAST_FUNC create_and_bind_stream_or_die(const char* bindaddr, int port) { return create_and_bind_or_die(bindaddr, port, SOCK_STREAM); }

int FAST_FUNC create_and_bind_dgram_or_die(const char* bindaddr, int port) { return create_and_bind_or_die(bindaddr, port, SOCK_DGRAM); }
...
static char* FAST_FUNC sockaddr2str(const struct sockaddr* sa, int flags) {
    ...
    if (rc) return NULL;
    if (flags & IGNORE_PORT) return xstrdup(host);
#if ENABLE_FEATURE_IPV6
    if (sa->sa_family == AF_INET6) {
...
char* FAST_FUNC xmalloc_sockaddr2host(const struct sockaddr* sa) { return sockaddr2str(sa, 0); }

char* FAST_FUNC xmalloc_sockaddr2host_noport(const struct sockaddr* sa) { return sockaddr2str(sa, IGNORE_PORT); }

char* FAST_FUNC xmalloc_sockaddr2hostonly_noport(const struct sockaddr* sa) { return sockaddr2str(sa, NI_NAMEREQD | IGNORE_PORT); }
#ifndef NI_NUMERICSCOPE
#define NI_NUMERICSCOPE 0
#endif
char* FAST_FUNC xmalloc_sockaddr2dotted(const struct sockaddr* sa) { return sockaddr2str(sa, NI_NUMERICHOST | NI_NUMERICSCOPE); }
